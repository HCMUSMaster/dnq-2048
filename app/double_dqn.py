...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Train and evaluate Double DQN on OpenSpiel 2048"
    )
    parser.add_argument(
        "--seed",
        type=int,
    ...
    parser.add_argument(
        "--buffer_size", type=int, default=50_000, help="Replay buffer capacity."
    )
    parser.add_argument(
        "--batch_size", type=int, default=128, help="Mini-batch size for optimization."
    )
    parser.add_argument(
        "--gamma", type=float, default=0.99, help="Discount factor for Bellman targets."
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--eps_start", type=float, default=1.0, help="Initial epsilon for exploration."
    )
    parser.add_argument(
        "--eps_end", type=float, default=0.05, help="Final epsilon after decay."
    )
    parser.add_argument(
        "--eps_decay_steps",
        type=int,
...
def train(args: argparse.Namespace, device: torch.device):
    ...
    for episode in tqdm(range(1, args.num_episodes + 1), desc="Training"):
        ...
        while not done and ep_len < args.max_steps_per_episode:
            ...
            with torch.no_grad():
                q_vals = q_net(
                    torch.tensor(np.asarray([obs]), dtype=torch.float32, device=device)
                )
